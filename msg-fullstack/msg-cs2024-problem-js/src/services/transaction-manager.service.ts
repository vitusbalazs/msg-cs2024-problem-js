...
export class TransactionManagerService {
  // transfer fromAccount: USD -> toAccount: EUR -> 10 RON
  ...
  public retrieveTransactionsByDate(accountId: string, date: Date) {
    if (!AccountsRepository.exist(accountId)) {
      throw new Error('Specified account does not exist');
    }
    return AccountsRepository.get(accountId)!.transactions.filter(transaction => {
      return dayjs(transaction.timestamp).isSame(dayjs(date), 'day');
    });
  }
}
